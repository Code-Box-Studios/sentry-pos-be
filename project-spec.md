...
- **Repositories:** three, split by deployment surface — `sentry-pos-fe` (the POS terminal, a static Next export at the repo root), `sentry-pos-landing` (the marketing site and its Payload CMS, server-rendered, needs Postgres) and `sentry-pos-be` (the NestJS API). The spec documents are mirrored at the root of each repo that needs them.
